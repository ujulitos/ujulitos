...
function loadXML(fileName) {
    ...
    $.ajax({
        ...
        error: function(request, status, error) {
            console.log('No se cargaron los datos!', request, status, error);
        }
    }); //ajax index
} //function loadXML
...
function loadPage() {
    ...
    for (var i = 0; i < getMax(); i++) {
        ...
        $(".fotof_" + personas[i].id).css({
            "background-image": " url(http://ujulitos.alwaysdata.net/images/" + personas[i].foto + ")"
        });
...
function cargaIframe() {
    //   $(".organigramaFrame").empty();
    //  $(".organigramaFrame").html('<iframe name="organigramaInt" class="organigramaInt" src="http://intelego.com.mx/cm/b/organigrama.xml" frameborder="0" scrolling="yes" border="1"></iframe>');
    ...
    // $('.organigramaFrame').load('http://intelego.com.mx/cm/b/organigrama.xml');
    loadXML('http://ujulitos.alwaysdata.net/organigrama.xml');
}
